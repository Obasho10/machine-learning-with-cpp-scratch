...
public:
    ...
    Model(double lambda, LossType lossType) {
        this->lambda = lambda;
    ...
    void forwardPass() {
        ...
        for (auto& layer : layers) {
            ...
            currentInput = layer->actoutputs;  // Output of current layer is input to the next layer
        }
    }
    ...
    std::vector<double> output = layers.back()->actoutputs;  // Assuming there's a method to get the output of the last layer
    ...
    for (int l = layers.size() - 1; l > 0; --l) {
        ...
        inputGradients = layers[l]->backprop(inputGradients, input, learningRate,lambda);
        //std::cout<<inputGradients[0]<<std::endl;
    }
    layers[0]->backprop(inputGradients,inputs,learningRate,lambda);
    ...
    // Set the inputs for the model    void Model()
    inputs = input;
    ...
    for (const auto& output : trueValues) {
            std::cout << output << " ";
        }
        std::cout << std::endl;
    layers[layers.size()-1]->printOutputs();
    return mse;
}
